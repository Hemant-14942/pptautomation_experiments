...
def format_qpill_qtext_image_mcq_slide(slide: Slide, prs: Presentation | None = None, dspec=None) -> bool:
    ...
    for opt in shapes["mcq_options"]:
        ...
        pill_el = opt["pill_el"]
        clone = format_mcq_option_pill(opt_idx, pill_el, dspec)
        if clone is not None:
            replace_shape(sp_tree, pill_el, clone)
            label_el = opt["label_el"]
            if label_el is not None and not opt["grouped"]:
                replace_shape(
                    sp_tree, label_el, format_mcq_option_label(opt_idx, label_el, dspec)
                )

        text_el = opt["text_el"]
        if text_el is not None:
            replace_shape(sp_tree, text_el, format_mcq_answer_text(opt_idx, text_el, dspec))
