"""Fixed layout for question pill + question text + multiple images (no MCQ, no table)."""
...
# Image area: ~80% slide width, centered under the question text.
...
def image_area_for(dspec=None) -> dict:
    """Image area below the question text, running down to the slide bottom margin.

    Defence-aware: question_text_for already shifts down when the template
    reserves a top banner, so this area follows automatically.
    """
...
def image_boxes_for(num_images: int, dspec=None) -> list[dict[str, int]]:
    ...
    Reuses the same grid layout (side-by-side / 2x2 / auto-grid) as
    image_only, so each image is naturally shorter than the single-image
    case as more images are added.
    """
    area = image_area_for(dspec)
    return get_image_boxes(num_images, content_area=area)
