"""Test the table_only formatter.

Run on one slide manually:
    python -m app.core.fit_layout.layout_formatter.table_only.tests.test_formatter <pptx_path> <slide_index>
"""
...
from app.core.fit_layout.layout_formatter.table_only.constants import TABLE_BOX
from app.core.fit_layout.layout_formatter.table_only.formatter import (
    calculate_table_x,
    calculate_table_y,
...
def verify_slide(pptx_path: str, slide_index: int) -> dict:
    """Check detection + formatter placement for one slide."""
    prs = Presentation(pptx_path)
    slide = prs.slides[slide_index]
    sig = parse_input_slide_signature(pptx_path)[slide_index]

    detected_type = detect_slide_type(sig, slide, prs)
    if detected_type != SLIDE_TYPE_TABLE_ONLY:
        return {
            "pass": False,
            "reason": f"slide type detected as {detected_type!r}, not table_only",
    ...
    return {
        ...
        "placement": "centered" if fits_in_box else "top-left anchored",
        "off": new_off,
        "ext": new_ext,
        "orig_ext": orig_ext,
...
def save_formatted_slide(pptx_path: str, slide_index: int, output_dir: str = OUTPUT_DIR) -> str | None:
    ...
    if detect_slide_type(sig, slide, prs) != SLIDE_TYPE_TABLE_ONLY:
        return None
...
def _print_report(pptx_path: str, slide_index: int, report: dict) -> None:
    ...
    w_in = Emu(report["table_width"]).inches
    off_in = (Emu(report["off"][0]).inches, Emu(report["off"][1]).inches)
    ext_in = (Emu(report["ext"][0]).inches, Emu(report["ext"][1]).inches)

    print(f"  table: {w_in:.2f}x{h_in:.2f}in  fits_in_box={report['fits_in_box']}  placement={report['placement']}")
    print(f"  off={off_in[0]:.2f},{off_in[1]:.2f}in  ext={ext_in[0]:.2f}x{ext_in[1]:.2f}in")
