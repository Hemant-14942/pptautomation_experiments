"""Shared helpers to locate question-pill slide shapes on input decks.

Question text rule (no minimum character length):
  - First text box below the question pill (len >= 1)
  - Exclude the pill label ("Question") and MCQ option letters (A-D)
  - On MCQ slides: must sit above the first option-pill row

Misaligned input positions are OK — formatters snap clones to fixed layout boxes.
"""
...
# Answers may sit slightly above the option-pill top in messy inputs.
...
def question_pill_bottom_emu(slide: Slide) -> int | None:
    """Bottom edge (EMU) of the question roundRect pill, or None."""
...
def first_mcq_option_top_emu(slide: Slide) -> int | None:
    """Top edge (EMU) of the highest MCQ option pill (ellipse or group)."""
    tops: list[int] = []
    for sp in slide.shapes:
        elem = sp._element
...
def find_question_text_element(
    ...
    *,
    ...
    """Return the question text shape: topmost text below the pill (len >= 1)."""
...
def find_mcq_answer_text_elements(
    ...
    """Answer text boxes in the MCQ zone (sorted top to bottom), up to 4."""
...
def has_question_text_below_pill(slide: Slide) -> bool:
    """True if any question text exists below the pill (for slide-type detection)."""
