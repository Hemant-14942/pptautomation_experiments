"""Test qpill_qtext_image_mcq formatter.

Run one slide:
    python -m app.core.fit_layout.layout_formatter.qpill_qtext_image_mcq.tests.test_formatter app/data/input/ptest1.pptx 15

Run all matching slides:
    python -m app.core.fit_layout.layout_formatter.qpill_qtext_image_mcq.tests.test_formatter app/data/input/ptest1.pptx
"""
...
from app.core.fit_layout.layout_formatter.qpill_qtext_image_mcq.formatter import (
    format_image,
...
def find_shapes(slide) -> dict | None:
    ...
    mcq_pill_els = [el for _, el in sorted(mcq_pill_els, key=lambda item: item[0])]
...
def save_formatted_slide(pptx_path: str, slide_index: int, output_dir: str = OUTPUT_DIR) -> str | None:
    ...
    if detect_slide_type(sig, slide, prs) != SLIDE_TYPE_QPILL_QTEXT_IMAGE_MCQ:
        return None
...
def run_all(pptx_path: str) -> None:
    ...
    for idx, slide in enumerate(prs.slides):
        if detect_slide_type(signatures[idx], slide, prs) != SLIDE_TYPE_QPILL_QTEXT_IMAGE_MCQ:
            continue
        matched += 1
    ...
    print(f"\n{matched} slides detected as qpill_qtext_image_mcq, {saved} formatted and saved to {OUTPUT_DIR}")
...
if __name__ == "__main__":
    if len(sys.argv) == 3:
        ...
        if detected != SLIDE_TYPE_QPILL_QTEXT_IMAGE_MCQ:
            sys.exit(1)
